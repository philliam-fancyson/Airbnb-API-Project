    ...
    return (
        <div>
            ...
            <div className="spot-callout-box">
                ...
                <p>{spot.avgStarRating ? <><FaStar /> {spot.avgStarRating.toFixed(2)} </>: <><FaStar />{"New"}</>} {spot.numReviews !== 0 ? <><LuDot /> {(spot.numReviews === 1 ? spot.numReviews + " review" : spot.numReviews + " reviews")} </>: null} </p>
                <button onClick={handleClick}>Reserve</button>
            </div>
            <div className="spot-reviews">
                <h3>{spot.avgStarRating ? <><FaStar /> {spot.avgStarRating.toFixed(2)} </>: <><FaStar />{"New"}</>} {spot.numReviews !== 0 ? <><LuDot /> {(spot.numReviews === 1 ? spot.numReviews + " review" : spot.numReviews + " reviews")} </>: null}</h3>
            </div>
        </div>
    )
}
